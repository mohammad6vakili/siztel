...
const RatingProfileCard = ({ rate, loadings }) => {
  const { skin } = useSkin();
  ...
  return (
    ...
    >
      ...
      >
        <div>Direction : {rate.Direction}</div>
        <div>Tenant : {rate.Tenant}</div>
        <div>Category : {rate.Category}</div>
        <div>Subject : {rate.Subject}</div>
        <div className="d-flex align-items-center">
          <Button
            onClick={() => navigate("/rules/rating_profiles/update")}
            style={{ marginRight: 8 }}
            color="primary"
            size="sm"
          >
